...
public class DAO {
	...
	public List<ProductsModels> getpage(int id,int cate) {
		List<ProductsModels> list = new ArrayList<>();
		String sql = "SELECT * FROM Products where categoryID=? and isSelling='True' and isActive='True'  ORDER BY productid OFFSET ? ROWS FETCH NEXT 10 ROWS ONLY";
	...
	public int countsp(int cateid) {
		String sql = "select count(*) from Products where categoryID=? and isSelling='True'";
		try {
			conn = new DBConnection().getConnection();
			ps = conn.prepareStatement(sql);
			ps.setInt(1, cateid);
