package vn.iotstar.controller.user.follow;

import java.io.IOException;
import java.util.List;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import vn.iotstar.dao.CateDAO;
import vn.iotstar.dao.UserDAO;
import vn.iotstar.model.CategoryModels;
import vn.iotstar.model.FollowModels;
import vn.iotstar.model.UserModels;

@WebServlet(urlPatterns = { "/listFollow" })
public class ListFollowController extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	CateDAO cate = new CateDAO();

	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		HttpSession session = req.getSession();
		int idAdmin = 0;
		try {
			idAdmin = (int) session.getAttribute("id");
			if (idAdmin == 0) {
				RequestDispatcher dispatcher = req.getRequestDispatcher("/view/user/dangnhap.jsp");
				dispatcher.forward(req, resp);
			}
		} catch (Exception e) {
			// TODO: handle exception
			RequestDispatcher dispatcher = req.getRequestDispatcher("/view/user/dangnhap.jsp");
			dispatcher.forward(req, resp);

		}
		UserDAO userDao = new UserDAO();
		UserModels admin = userDao.get(idAdmin);
		req.setAttribute("id", admin);

		

		String indexPage = req.getParameter("index");

		if (indexPage == null) {
			indexPage = "1";
		}
		int indexp = Integer.parseInt(indexPage);
		int countP = userDao.countListFollow(idAdmin);
		int endPage = countP / 10;
		if (countP / 10 != 0) {
			endPage++;
		}
		List<FollowModels> list = userDao.getListFollow(idAdmin,indexp-1);
		req.setAttribute("listfollow", list);
		req.setAttribute("countProductAll", countP);

		req.setAttribute("endP", endPage);
		req.setAttribute("tag", indexp);

		/*
		 * CategoryModels a= new CategoryModels(); a=cateList.get(1); int d=0; if
		 * (a.isDel()==true) {d=1;} req.setAttribute("aa", d);
		 */
		RequestDispatcher dispatcher = req.getRequestDispatcher("/view/user/listFollow.jsp");
		dispatcher.forward(req, resp);

	}
}
