...
public class EditCategoryController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		CategoryModels category = cateDAO.get(Integer.parseInt(id));
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			for (FileItem item : items) {
				if (item.getFieldName().equals("cateid")) {
				...
				} else if (item.getFieldName().equals("namecate")) {
					...
					category.setIsDel(Boolean.parseBoolean(item.getString("UTF-8")));
				}else if (item.getFieldName().equals("createCateDate")) {
					category.setCreateCateDate(item.getString("UTF-8"));
				}else if (item.getFieldName().equals("updateCateDate")) {
					category.setUpdateCateDate(item.getString("UTF-8"));
				}	
			}
